...
import com.zige.robot.fsj.model.event.MsgBean;
...
public class CallManager {
    ...
    public void init(Context context) {
        ...
        // 设置音视频通话采样率，一般不需要设置，为了减少噪音，可以将采样率适当调低，这里默认设置32k
    ...
    public void saveCallMessage() {
        Log.d(App.TAG, "The call ends and the call log message is saved! " + endType);
        ...
        message.setAttribute("callTime", callTime);//添加个属性到message中
    ...
    public void endCall() {
        try {
            ...
            e.printStackTrace();
            Log.e(App.TAG, "结束通话失败：error %d - %s" + e.getErrorCode() + e.getMessage());
        }
        // 挂断电话调用保存消息方法
        saveCallMessage();
        // 通话结束，重置通话状态
        ...
        if (isLoaded) {
            ...
            soundPool.setOnLoadCompleteListener(new SoundPool.OnLoadCompleteListener() {
                ...
                public void onLoadComplete(SoundPool soundPool, int i, int i1) {
                    Log.d(App.TAG, "SoundPool load complete! loadId: %d" + loadId);
                    isLoaded = true;
                    // 首次监听到加载完毕，开始播放音频
                    playCallSound();
                }
