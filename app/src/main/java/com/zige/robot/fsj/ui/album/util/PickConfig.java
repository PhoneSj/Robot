package com.zige.robot.fsj.ui.album.util;


import com.zige.robot.R;

/**
 * Created by wanbo on 2016/12/30.
 * PickPhotoView 配置文件
 */

public class PickConfig {

    // TAG
    public static final String TAG = "PickPhotoView";
    // intent data
    public static final String INTENT_PICK_DATA = "intent_pick_Data";
    // intent dirName
    public static final String INTENT_DIR_NAME = "intent_dir_name";
    // intent img path
    public static final String INTENT_IMG_PATH = "intent_img_path";
    // intent img list
    public static final String INTENT_IMG_LIST = "intent_img_list";
    // intent camera uri
    public static final String INTENT_CAMERA_URI = "intent_camera_uri";
    // intent img select list
    public static final String INTENT_IMG_LIST_SELECT = "intent_img_list_select";

    //
    public static final String INTENT_SELECT_REMOTE_ALBUM_NAME = "intent_select_remote_album_name";
    public static final String INTENT_SELECT_REMOTE_ALBUM_ID = "intent_select_remote_album_id";


    // all photos
    public static final String ALL_PHOTOS = "All Photos";
    // Camera type
    public static final int CAMERA_TYPE = -1;
    // space
    public static final int ITEM_SPACE = 4;
    // intent requestCode
    public static final int PICK_PHOTO_DATA = 0x5521;
    // intent requestCode
    public static final int LIST_PHOTO_DATA = 0x0821;
    // intent requestCode
    public static final int CAMERA_PHOTO_DATA = 0x9949;
    // intent requestCode
    public static final int PREVIEW_PHOTO_DATA = 0x7763;
    // default size
    public static final int DEFAULT_PICK_SIZE = 9;
    // default span count
    public static final int DEFAULT_SPAN_COUNT = 4;
    // list scroll threshold
    public static final int SCROLL_THRESHOLD = 30;
    // toolbar icon color
    public static final int PICK_BLACK_COLOR = R.color.pick_black;
    public static final int PICK_WHITE_COLOR = R.color.pick_white;

    public static int REQUEST_CODE_REMOTE_ALBUM = 0xe000;
    public static int REQUEST_CODE_PHOTO_PREVIEW = 0xe001;
    public static int REQUEST_CODE_REMOTE_DIR = 0xe002;

    public static int RESULT_CODE_REMOTE_ALBUM = 0xf000;
    public static int RESULT_CODE_PHOTO_PREVIEW = 0xf001;
    public static int RESULT_CODE_REMOTE_DIR = 0xf002;

    public static String INTENT_PHOTO_CURRENT = "intent_photo_current";
    public static String INTENT_PHOTO_ALL = "intent_photo_all";
    public static String INTENT_ALBUM_ID = "intent_album_id";
    public static String INTENT_ALBUM_NAME = "intent_album_name";
    public static String INTENT_PHOTO_POSITION = "intent_photo_position";

}
