...
public class MediaRecorderManager {
    ...
    public long getVoice_time_length() {
        return voice_time_length;
    }

    public void setVoice_time_length(long voice_time_length) {
        this.voice_time_length = voice_time_length;
    }

    private long voice_time_length;
    /**
     * 单例化这个类
     */
    private static MediaRecorderManager mInstance;

    private MediaRecorderManager(String dir) {
        mDirString = dir;
    }
    ...
    public static MediaRecorderManager getInstance(String dir) {
        if (mInstance == null) {
            synchronized (MediaRecorderManager.class) {
                if (mInstance == null) {
                    mInstance = new MediaRecorderManager(dir);
                }
            }
        }
        return mInstance;
    ...
    public void prepareAudio() {
        try {
            ...
            if (mListener != null) {
                ...
                Log.d(TAG, "prepareAudio: start_time:  " + startTime);
                mListener.wellPrepared(); //准备完成回调
            }
    ...
    public void release() {
        ...
        * 这里处理一些特定情况下的异常。2017/04/12 by wgyscsf
        ...
        try {
            ...
            Log.d(TAG, "prepareAudio: end_time:  " + System.currentTimeMillis());
            voice_time_length = (System.currentTimeMillis() - startTime) / 1000; //语音长度
